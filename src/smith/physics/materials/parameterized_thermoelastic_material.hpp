...
/**
 * @brief Thermal-Stiffening material model
 *
 */
...
// Linear model
  ...
  using std::pow, std::exp;
    ...
    // Mechanical part: lambda tr(eps) I + 2 mu eps
    auto sigma_mech = lambda * tr_eps * I + (2.0 * mu) * eps;

    // Thermal stress: -3 K beta (theta - Tr) I
...
/**
 * @brief Thermal-Stiffening material model without internal mass fraction
 *
 */
