...
struct ThermoMechanicsSystem : public SystemBase {
  ...
  static_assert(TemperatureTimeRule::num_states == 2,
  ...
  using SolidWeakFormType = TimeDiscretizedWeakForm<
      ...
      TimeRuleParams2<DisplacementTimeRule, H1<disp_order, dim>, TemperatureTimeRule, H1<temp_order>,
                      parameter_space...>>;

  /// @brief using for ThermalWeakFormType
  using ThermalWeakFormType = TimeDiscretizedWeakForm<
      dim, H1<temp_order>,
      TimeRuleParams2<TemperatureTimeRule, H1<temp_order>, DisplacementTimeRule, H1<disp_order, dim>,
                      parameter_space...>>;
  ...
  void setMaterial(const MaterialType& material, const std::string& domain_name)
  {
    ...
    solid_weak_form->addBodyIntegral(
        domain_name, [=](auto t_info, auto /*X*/, auto u, auto u_old, auto v_old, auto a_old, auto temperature,
          ...
          auto [pk, C_v, s0, q0] = material(t_info.dt(), state, get<DERIVATIVE>(u_current),
                                            get<DERIVATIVE>(v_current), get<VALUE>(T), get<DERIVATIVE>(T), params...);
          return smith::tuple{zero{}, pk};
        });

    thermal_weak_form->addBodyIntegral(
        domain_name, [=](auto t_info, auto /*X*/, auto T, auto T_old, auto disp, auto disp_old, auto v_old,
    ...
    cycle_zero_weak_form->addBodyIntegral(
        domain_name, [=](auto t_info, auto /*X*/, auto u, auto v, auto a, auto temperature,
                         auto temperature_old, auto... params) {
          auto T = captured_temp_rule->value(t_info, temperature, temperature_old);
                            ...
                            BodySourceType source_function)
  {
    ...
    thermal_weak_form->addBodySource(
        ...
        [=](auto t_info, auto X, auto T, auto T_old, auto disp, auto disp_old, auto v_old, auto a_old,
            auto... params) {
          auto [current_u, v_current, a_current] = captured_disp_rule->interpolate(t_info, disp, disp_old, v_old, a_old);
          auto [T_current, T_dot] = captured_temp_rule->interpolate(t_info, T, T_old);
  ...
  {
    addHeatSourceAllParams(domain_name, source_function,
                          ...
                          SurfaceFluxType flux_function)
  {
    ...
    thermal_weak_form->addBoundaryFlux(
        depends_on, domain_name,
        [=](auto t_info, auto X, auto n, auto T, auto T_old, auto disp, auto disp_old, auto v_old, auto a_old,
            auto... params) {
          auto [current_u, v_current, a_current] =
              captured_disp_rule->interpolate(t_info, disp, disp_old, v_old, a_old);
          auto [T_current, T_dot] = captured_temp_rule->interpolate(t_info, T, T_old);
          return -flux_function(t_info.time(), X, n, current_u, v_current, a_current, T_current, T_dot, params...);
        });
  }
 ...
 private:
  ...
  template <typename SurfaceFluxType, std::size_t... Is>
  void addHeatFluxAllParams(const std::string& domain_name, SurfaceFluxType flux_function,
                                   std::index_sequence<Is...>)
  {
