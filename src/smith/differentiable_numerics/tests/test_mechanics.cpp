...
std::vector<smith::FiniteElementState*> getStatePtrs(const std::vector<smith::FieldState>& field_states)
{
  std::vector<smith::FiniteElementState*> pointers;
...
struct MeshFixture : public testing::Test {
  ...
  void SetUp()
  {
    ...
    double length = 0.5;
    double width = 2.0;
    mesh = std::make_shared<smith::Mesh>(mfem::Mesh::MakeCartesian2D(2, 2, mfem_shape, true, length, width), MESHTAG, 0,
                                         0);
    // checkpointing graph
    ...
    smith::DoubleState fixed_dt = checkpointer->create_state<double, double>(1e-3);
    ...
    *density0.get() = density;
    ...
    auto solid_mass_residual = smith::create_solid_mass_weak_form<VectorSpace::components, VectorSpace, DensitySpace>(
    ...
    std::shared_ptr<smith::StateAdvancer> time_integrator =
        std::make_shared<smith::LumpedMassExplicitNewmark>(solid_mechanics_residual, solid_mass_residual, bc_manager);
    auto dt_estimator = std::make_shared<smith::ConstantTimeStepEstimator>(fixed_dt);

    // construct mechanics
    mechanics = std::make_shared<smith::DifferentiablePhysics>(mesh, checkpointer, *shape_disp, states, params,
                                                               time_integrator, dt_estimator, "mechanics");
    physics = mechanics;

    // kinetic energy integrator for qoi
    kinetic_energy_integrator = smith::create_kinetic_energy_integrator<VectorSpace, DensitySpace>(
...
TEST_F(MeshFixture, TRANSIENT_DYNAMICS_GRETL)
{
  ...
  gretl::State<double> gretl_qoi = smith::compute_kinetic_energy(kinetic_energy_integrator, *shape_disp,
  ...
  for (size_t m = 0; m < num_steps; ++m) {
    ...
    gretl_qoi = gretl_qoi + smith::compute_kinetic_energy(kinetic_energy_integrator, *shape_disp, all_fields[F_VELO],
  ...
  EXPECT_GT(smith::check_grad_wrt(gretl_qoi, *shape_disp, *checkpointer, 0.01, 4, true), 0.8);
  EXPECT_GT(smith::check_grad_wrt(gretl_qoi, initial_states[DISP], *checkpointer, 0.01, 4, true), 0.8);
  EXPECT_GT(smith::check_grad_wrt(gretl_qoi, initial_states[VELO], *checkpointer, 0.01, 4, true), 0.8);
  EXPECT_GT(smith::check_grad_wrt(gretl_qoi, initial_states[ACCEL], *checkpointer, 1.0, 4, true), 0.8);
  EXPECT_GT(smith::check_grad_wrt(gretl_qoi, initial_states[DENSITY], *checkpointer, 0.01, 4, true), 0.8);
}
