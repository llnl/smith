...
DoubleState evaluateObjective(const ScalarObjective& objective,
                              const FieldState& shape_disp, const std::vector<FieldState>& inputs, const TimeInfo& time_info)
{
  ...
  value.set_eval([time_info, objective_ptr](const gretl::UpstreamStates& upstreams, gretl::DownstreamState& downstream) {
  ...
  value.set_vjp([time_info, objective_ptr](gretl::UpstreamStates& upstreams, const gretl::DownstreamState& downstream) {
    ...
    upstreams[0].get_dual<FEDualPtr, FEFieldPtr>()->Add(
        downstream_dual, objective_ptr->mesh_coordinate_gradient(time_info, shape.get(), getConstFieldPointers(fields)));
    for (size_t i = 1; i < upstreams.size(); ++i) {
