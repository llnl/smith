...
/**
 * @brief Set a coupled thermo-mechanical internal variable material on the solid, thermal, and internal variable systems.
 *
 ...
 */
template <typename SolidSystem, typename ThermalSystem, typename InternalVariableSystem,
          typename ThermoMechMaterial, typename InternalVarEvolution>
void setCoupledThermoMechanicsInternalVariableMaterial(
    std::shared_ptr<SolidSystem> solid_system, std::shared_ptr<ThermalSystem> thermal_system,
    std::shared_ptr<InternalVariableSystem> internal_variable_system,
    ThermoMechMaterial thermo_mech_material, InternalVarEvolution internal_var_evolution,
    const std::string& domain_name)
{
  ...
  solid_system->solid_weak_form->addBodyIntegral(
      domain_name, [=](auto t_info, auto /*X*/, auto u, auto u_old, auto v_old, auto a_old,
        ...
        auto [pk, Cv, s0, q0] = thermo_mech_material(
            t_info, get<VALUE>(alpha), get<DERIVATIVE>(u_current), get<DERIVATIVE>(v_current),
  ...
  thermal_system->thermal_weak_form->addBodyIntegral(
      ...
      [=](auto t_info, auto /*X*/, auto T, auto T_old, auto disp, auto disp_old, auto v_old, auto a_old) {
        ...
        auto [pk, C_v, s0, q0] = thermo_mech_material(t_info, 0.0, get<DERIVATIVE>(u_current), get<DERIVATIVE>(v_current),
  ...
  // setCoupledThermoMechanicsInternalVariableMaterial(solid, thermal, internal_vars, thermo_mech_mat, internal_var_evo, domain);
  // So we just register the internal_var_evolution using `addEvolution`!
