...
void FiniteDifferenceParameter(LoadingType load, int sensitivity_parameter_index = 0)
{
  ...
  serac::StateManager::initialize(datastore, "thermomech_functional_parameterized_sensitivities");
  ...
  ThermomechanicsMonolithic<p, dim, Parameters<H1<1>, H1<1>, H1<1>>> thermomech_solver(
      nonlinear_opts, linear_opts, "thermomech_parameterized_sensitivities", MESHTAG,
  ...
  // // Debug
  // mfem::ParGridFunction adj_sensitivity(&user_defined_youngs_modulus.space(), &sensitivity);
  // mfem::ParGridFunction fd_sensitivity(user_defined_youngs_modulus.gridFunction());
  // mfem::ParGridFunction dqoi_dparam_temp(user_defined_youngs_modulus.gridFunction());
  // mfem::ParGridFunction dqoi_dparam_disp(user_defined_youngs_modulus.gridFunction());
  for (int i = 0; i < sensitivity_parameter->gridFunction().Size(); ++i) {
    ...
    // // Debug
    // fd_sensitivity(i) = dqoi_dparam;
    // dqoi_dparam_temp(i) = dqoi_dparam_1;
    // dqoi_dparam_disp(i) = dqoi_dparam_2;
  ...
  // // Debug
  // mfem::ParaViewDataCollection paraview_dc("sensitivity_debug", &pmesh);
  // paraview_dc.SetPrefixPath("sensitivity_debug");
  // paraview_dc.SetTime(1.0);
  // paraview_dc.RegisterField("fd_sensitivity", &fd_sensitivity);
  // paraview_dc.RegisterField("adj_sensitivity", &adj_sensitivity);
  // paraview_dc.RegisterField("dqoi_dparam_temp", &dqoi_dparam_temp);
  // paraview_dc.RegisterField("dqoi_dparam_disp", &dqoi_dparam_disp);
  // paraview_dc.Save();
}
