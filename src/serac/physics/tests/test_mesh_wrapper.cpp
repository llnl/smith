...
TEST(Mesh, TestMeshFromParMesh)
{
  ...
  serac::StateManager::initialize(datastore, "datastore");

  auto parMesh =
      mesh::refineAndDistribute(buildRectangleMesh(2, 3, 2, 3), serial_refinement, parallel_refinement, MPI_COMM_WORLD);
  auto mesh = std::make_shared<serac::Mesh>(std::move(*parMesh), "mesh_tag");
...
TEST(Mesh, TestMeshFromSerialMesh)
{
  ...
  auto mesh = std::make_shared<serac::Mesh>(buildCuboidMesh(8, 1, 1, L, W, H), "mesh_tag");
...
TEST(Mesh, TestMeshFromFile)
{
  ...
  auto mesh = std::make_shared<serac::Mesh>(filename, "mesh_tag", 0, 0, MPI_COMM_WORLD);
  ...
  mesh->addDomainOfBoundaryElements("domain1", serac::by_attr<dim>(1));
