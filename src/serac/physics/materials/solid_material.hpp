...
struct LinearIsotropic {
  ...
  SERAC_HOST_DEVICE auto operator()(State& /* state */, double /* dt */, const tensor<T, dim, dim>& du_dX) const
  {
    auto I       = Identity<dim>();
...
struct StVenantKirchhoff {
   ...
   */
  template <typename T, int dim>
  auto operator()(State&, double /* dt */, const tensor<T, dim, dim>& grad_u) const
  {
