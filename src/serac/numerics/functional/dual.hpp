...
SERAC_HOST_DEVICE constexpr auto inner(const dual<S>& A, const dual<T>& B) { return A * B; }
 ...
 */
template <typename T>
SERAC_HOST_DEVICE constexpr auto inner(double A, const dual<T>& B) { return A * B; }
 ...
 */
template <typename S>
SERAC_HOST_DEVICE constexpr auto inner(const dual<S>& A, double B) { return A * B; }
