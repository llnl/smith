...
class HomotopySolver {
 ...
 public:
  ...
  void PredictorResidual(const BlockVector& X, const double theta, const double thetaplus, BlockVector& r,
                         int& reval_err);
  ...
  void DogLeg(const BlockOperator& JkOp, const BlockVector& gk, const double delta, const BlockVector& dXN,
              BlockVector& dXtr);
  bool FilterCheck(const Vector& r_comp_norm);
  void UpdateFilter(const Vector& r_comp_norm);
  void ClearFilter();
  bool NeighborhoodCheck(const BlockVector& X, const BlockVector& r, const double theta, const double beta,
                         double& betabar_);
  bool NeighborhoodCheck_1(const BlockVector& X, const BlockVector& r, const double theta, const double beta,
                           double& betabar_);
  bool NeighborhoodCheck_2(const BlockVector& X, const BlockVector& r, const double theta, const double beta,
                           double& betabar_);
