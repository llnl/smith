...
HomotopySolver::HomotopySolver(GeneralNLMCProblem* problem_)
    : problem(problem_),
      block_offsets_xsy(4),
      dFdx(nullptr),
      dFdy(nullptr),
...
{
  BlockVector Xk(block_offsets_xsy);
  Xk = 0.0;
  BlockVector Xtrial(block_offsets_xsy);
  Xtrial = 0.0;
  BlockVector dXNk(block_offsets_xsy);
  dXNk = 0.0;
  BlockVector dXtrk(block_offsets_xsy);
  dXtrk = 0.0;
  BlockVector GX(block_offsets_xsy);
  GX = 0.0;
  BlockVector GX0(block_offsets_xsy);
  GX0 = 0.0;
  BlockVector rk(block_offsets_xsy);
  rk = 0.0;
  BlockVector rktrial(block_offsets_xsy);
  rktrial = 0.0;
  BlockVector rklinear(block_offsets_xsy);
  rklinear = 0.0;
  BlockVector gk(block_offsets_xsy);
  gk = 0.0;  // grad(mk) = grad_z(||rk + Jk * z||)|_{z=0} = 2 Jk^T rk
  ...
  MFEM_VERIFY(Feval_err == 0 && Qeval_err == 0,
              "unsuccessful evaluation of F and/or Q at initial point of Homotopy solver");
  ...
  while (jOpt < max_outer_iter) {
    ...
    JGX.MultTranspose(rk, gk);
    gk *= 2.0;  // gradient of quadratic-model (\nabla_{dX}(||rk + Jk * dX||_2)^2)_{|dX=0}= 2 Jk^T rk
    ...
    Vector rktrial_comp_norm(3);
    rktrial_comp_norm = 0.0;
    for (int i = 0; i < 3; i++) {
    ...
    if (inNeighborhood) {
      ...
      BlockVector rkp(block_offsets_xsy);
      rkp = 0.0;
      BlockVector dXp(block_offsets_xsy);
      dXp = 0.0;
      BlockVector Xtrialp(block_offsets_xsy);
      Xtrialp = 0.0;
...
double HomotopySolver::E(const BlockVector& X, int& Eeval_err)
{
  ...
  BlockVector r0(block_offsets_xsy);
  r0 = 0.0;
...
void HomotopySolver::PredictorResidual(const BlockVector& X, const double theta, const double thetaplus, BlockVector& r,
                                       int& reval_err)
{
  G(X, theta, r, reval_err);
...
void HomotopySolver::JacG(const BlockVector& X, const double theta, BlockOperator& JacG)
{
  ...
  Vector diagJacGxx(dimx);
  diagJacGxx = 0.0;
  for (int i = 0; i < dimx; i++) {
    diagJacGxx(i) = 1.0 - (X(i) - X(i + dimx)) / sqrt(pow(X(i) - X(i + dimx), 2) + 4. * pow(theta * ax(i), q));
  }
  if (JGxx) {
    delete JGxx;
    JGxx = nullptr;
  ...
  JacG.SetBlock(0, 0, JGxx);
  JacG.SetBlock(0, 1, JGxs);
  JacG.SetBlock(1, 0, JGsx);
  JacG.SetBlock(1, 1, JGss);
  JacG.SetBlock(1, 2, JGsy);
  JacG.SetBlock(2, 0, JGyx);
  JacG.SetBlock(2, 2, JGyy);
}
...
void HomotopySolver::DogLeg(const BlockOperator& JkOp, const BlockVector& gk, const double delta,
                            const BlockVector& dXN, BlockVector& dXtr)
{
  ...
  if (dXN_norm <= delta) {
    ...
    BlockVector dXsd(block_offsets_xsy);
    dXsd = 0.0;
    BlockVector Jkgk(block_offsets_xsy);
    Jkgk = 0.0;
    JkOp.Mult(gk, Jkgk);
    double gk_norm = GlobalLpNorm(2, gk.Norml2(), MPI_COMM_WORLD);
...
bool HomotopySolver::FilterCheck(const Vector& r_comp_norm)
{
  ...
  Array<bool> reductionJ;
  reductionJ.SetSize(3);
  for (int j = 0; j < 3; j++) {
    reductionJ[j] = true;
  }
...
bool HomotopySolver::NeighborhoodCheck(const BlockVector& X, const BlockVector& r, const double theta,
                                       const double beta, double& betabar_)
{
  if (useNeighborhood1) {
...
bool HomotopySolver::NeighborhoodCheck_1([[maybe_unused]] const BlockVector& X, const BlockVector& r,
                                         const double theta, const double beta, double& betabar_)
{
...
bool HomotopySolver::NeighborhoodCheck_2(const BlockVector& X, const BlockVector& r, const double theta,
                                         const double beta, double& betabar_)
{
  ...
  bool inNeighborhood =
      ((r_inf_norms(0) <= beta * theta * xs_inf_norm) && (max(r_inf_norms(1), r_inf_norms(2)) <= beta * theta));
  betabar_ = max(r_inf_norms(0) / (theta * xs_inf_norm), max(r_inf_norms(1), r_inf_norms(2)) / theta);
  return inNeighborhood;
}

void HomotopySolver::SetLinearSolver(Solver& solver_) { linSolver = &(solver_); }
