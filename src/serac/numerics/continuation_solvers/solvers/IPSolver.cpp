...
{
  BlockVector x0block(block_offsetsx);
  x0block = 0.0;
  ...
  // }
  BlockVector xfblock(block_offsetsx);
  xfblock = 0.0;
...
void ParInteriorPointSolver::Mult(const BlockVector& x0, BlockVector& xf)
{
  ...
  BlockVector xk(block_offsetsx), xhat(block_offsetsx);
  xk = 0;
  xhat = 0.0;
  BlockVector Xk(block_offsetsumlz), Xhat(block_offsetsumlz);
  Xk = 0.0;
  Xhat = 0.0;
  BlockVector Xhatuml(block_offsetsuml);
  Xhatuml = 0.0;
  Vector zlhat(dimM);
  zlhat = 0.0;
...
void ParInteriorPointSolver::IPNewtonSolve(BlockVector& x, Vector& l, Vector& zl, Vector& zlhat, BlockVector& Xhat,
                                           double mu)
{
  // solve A x = b, where A is the IP-Newton matrix
  BlockOperator A(block_offsetsuml, block_offsetsuml);
  BlockVector b(block_offsetsuml);
  b = 0.0;
  ...
  // b = - [grad_m phi + Jm^T l]
  //       [          c        ]
  BlockVector gradphi(block_offsetsx);
  gradphi = 0.0;
  BlockVector JTl(block_offsetsx);
  JTl = 0.0;
  Dxphi(x, mu, gradphi);
...
#else
    MFEM_ERROR(
        "IP-Newton linear solve failure due to not having compiled with STRUMPACK, MUMPS or MKL_CPARDISO or having set "
        "the linear solver of the IP solver via the");
#endif
#endif
#endif
...
void ParInteriorPointSolver::lineSearch(BlockVector& X0, BlockVector& Xhat, double mu)
{
  ...
  BlockVector x0(block_offsetsx);
  x0 = 0.0;
  x0.GetBlock(0).Set(1.0, u0);
  x0.GetBlock(1).Set(1.0, m0);

  BlockVector xhat(block_offsetsx);
  xhat = 0.0;
  xhat.GetBlock(0).Set(1.0, uhat);
  xhat.GetBlock(1).Set(1.0, mhat);

  BlockVector xtrial(block_offsetsx);
  xtrial = 0.0;
  BlockVector Dxphi0(block_offsetsx);
  Dxphi0 = 0.0;
  int maxBacktrack = 20;
  alpha = alphaMax;
  ...
  for (int i = 0; i < maxBacktrack; i++) {
    ...
    if (iAmRoot) {
      cout << "| grad(phi)^xhat - (phi(x0 + alpha xhat) - phi(x0)) / alpha | = "
           << abs(Dxphi0_xhat - (phxtrial - phx0) / alpha) << ", alpha = " << alpha << endl;
    }
    ...
    if (!inFilterRegion) {
      ...
      if (iAmRoot) {
        ...
        cout << "phi(xtrial) = " << phxtrial << ", phi(x0) = " << phx0
             << ", phi(x0) - gPhi *theta(x0) = " << phx0 - gPhi * thx0 << endl;
      }
      // Case I
      if (thx0 <= thetaMin && switchCondition) {
        ...
        if (sufficientDecrease) {
          ...
          if (iAmRoot) {
            cout << "sufficient decrease not achieved in log-barrier objective.\n";
          }
        }
      } else {
...
void ParInteriorPointSolver::DxL(const BlockVector& x, const Vector& l, const Vector& zl, BlockVector& y)
{
  ...
  BlockVector gradxf(block_offsetsx);
  gradxf = 0.0;
...
bool ParInteriorPointSolver::GetConverged() const { return converged; }

void ParInteriorPointSolver::SetTol(double Tol) { OptTol = Tol; }

void ParInteriorPointSolver::SetMaxIter(int max_it) { max_iter = max_it; }

void ParInteriorPointSolver::SetBarrierParameter(double mu_0) { mu_k = mu_0; }
...
void ParInteriorPointSolver::GetLogBarrierU(Vector& uLogBar) { uLogBar.Set(1.0, uLogBarrierSol); }

void ParInteriorPointSolver::GetLogBarrierM(Vector& mLogBar) { mLogBar.Set(1.0, mLogBarrierSol); }

void ParInteriorPointSolver::GetLogBarrierL(Vector& lLogBar) { lLogBar.Set(1.0, lLogBarrierSol); }

void ParInteriorPointSolver::GetLogBarrierZl(Vector& zlLogBar) { zlLogBar.Set(1.0, zlLogBarrierSol); }

void ParInteriorPointSolver::GetNumIterations(int& its) { its = jOpt; }

void ParInteriorPointSolver::GetLogBarrierMu(double& mu) { mu = muLogBarrierSol; }

void ParInteriorPointSolver::SetLogBarrierMu(double& mu) { muLogBarrierSol = mu; }

void ParInteriorPointSolver::SetLinearSolver(Solver& solver_) { linSolver = &(solver_); }
