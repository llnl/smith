...
ParGeneralOptProblem::~ParGeneralOptProblem() { block_offsetsx.DeleteAll(); }
...
double ParOptProblem::CalcObjective(const BlockVector& x, int& eval_err) { return E(x.GetBlock(0), eval_err); }
...
HypreParMatrix* ParOptProblem::Duuf(const BlockVector& x) { return DddE(x.GetBlock(0)); }
...
HypreParMatrix* ParOptProblem::Duc(const BlockVector& x) { return Ddg(x.GetBlock(0)); }

HypreParMatrix* ParOptProblem::Dmc([[maybe_unused]] const BlockVector& x) { return Ih; }
...
double ReducedProblem::E(const Vector& d, int& eval_err) { return problem->E(d, eval_err); }

// gradient of energy objective
void ReducedProblem::DdE(const Vector& d, Vector& gradE) { problem->DdE(d, gradE); }

HypreParMatrix* ReducedProblem::DddE(const Vector& d) { return problem->DddE(d); }

void ReducedProblem::g(const Vector& d, Vector& gd, int& eval_err)
{
  Vector gdfull(problem->GetDimM());
  gdfull = 0.0;
