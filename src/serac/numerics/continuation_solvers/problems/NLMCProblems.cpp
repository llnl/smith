...
void OptNLMCProblem::F(const Vector& x, const Vector& y, Vector& feval, int& eval_err) const
{
  MFEM_VERIFY(x.Size() == dimx && y.Size() == dimy && feval.Size() == dimx,
              "OptNLMCProblem::F -- Inconsistent dimensions");
...
void OptNLMCProblem::Q(const Vector& x, const Vector& y, Vector& qeval, int& eval_err) const
{
  MFEM_VERIFY(x.Size() == dimx && y.Size() == dimy && qeval.Size() == dimy,
              "OptNLMCProblem::Q -- Inconsistent dimensions");
...
// dF/dx = 0
HypreParMatrix* OptNLMCProblem::DxF([[maybe_unused]] const Vector& x, [[maybe_unused]] const Vector& y) { return dFdx; }

// dF/dy = dg/dy
HypreParMatrix* OptNLMCProblem::DyF([[maybe_unused]] const Vector& x, const Vector& y) { return optproblem->Ddg(y); }
...
HypreParMatrix* OptNLMCProblem::DyQ([[maybe_unused]] const Vector& x, const Vector& y) { return optproblem->DddE(y); }
