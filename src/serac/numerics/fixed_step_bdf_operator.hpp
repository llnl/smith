 ...
 *
 * @tparam q_order The maximum order of the BDF method (e.g., 1 to 6).
 */
template <int q_order = 2>
struct FixedStepBDFOperator {
  ...
  int available_order = std::min(k_plus_1, q_order);
   ...
   */
  static int constexpr order() { return q_order; }

private:
  /**
   * @brief Internal function to compute all β_j coefficients for a given order.
   *
   * @param order The order of the BDF method (≤ q_order).
