...
class SolidMechanicsStateAdvancer2 : public StateAdvancer {
 public:
                                                ...
                                                const TimeInfo& time_info) const override
  {
    ...
    solid_inputs.insert(solid_inputs.end(), params.begin(), params.end());
    return {evaluateWeakForm(weak_form_->final_reaction_weak_form, time_info, shape_disp, solid_inputs,
                             states[DISPLACEMENT])};
  }
...
int main(int argc, char* argv[])
{
  ...
  auto [physics, weak_form, bcs] =
      buildSolidMechanics<dim, ShapeDispSpace, VectorSpace, VectorSpace, ScalarParameterSpace, ScalarParameterSpace>(
          mesh, d_solid_nonlinear_solver, time_rule, physics_name, {"u_prev", "bulk", "shear"});
  ...
  weak_form->addBodyIntegral(smith::DependsOn<0, 1, 2>{}, mesh->entireBodyName(),
                             [material](const auto& time_info, auto /*X*/, auto u, auto /*v*/, auto /*a*/, auto u_prev,
                                        auto bulk, auto shear) {
                               MaterialType::State state;
                               auto du_dX = get<DERIVATIVE>(u);
                               auto v = (1.0 / time_info.dt()) * (u - u_prev);
  ...
  *(params[0].get()) = 0.0;
  ...
  params[2].get()->setFromFieldFunction([=](smith::tensor<double, dim>) {
    double scaling = 1.0;
