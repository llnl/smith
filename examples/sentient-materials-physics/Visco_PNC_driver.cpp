...
using ViscoThermalStiffeningMaterial = extended_thermomechanics_materials::ViscoThermalStiffeningMaterial;
static constexpr int Statedim = ViscoThermalStiffeningMaterial::FullStatePacking<dim>::packed_size;
...
ViscoThermalStiffeningMaterial makeViscoThermalStiffeningMaterial()
{
  double K = 0.5e6;
  double beta = 0.0;
  double Gmeq0 = 1.;//0.0073976;
  double Gmneq0 = 1.;//0.0073976;
  double etam = 1e4;
  double Gceq0 = 1.;//0.225075;
  double Gcneq0 = 1.;//0.225075;
  double etac = 1e4;
  double Jcm = 1.0;
  double C_v = 1.5;
  double kappa = 30.0;
  ...
  return ViscoThermalStiffeningMaterial{K,     beta,    Gmeq0,    Gmneq0, etam, Gceq0, Gcneq0,
                                        etac,  Jcm,     C_v,      kappa,  rho0, c,     modscale,
                               ...
                               GmresBlockPreconditioner gmres_block_preconditioner)
{
  ...
  system.disp_bc->setVectorBCs<dim>(mesh->domain("back"), std::vector<int>{2},
                                                [](double t, auto X) {
                                                  auto output = 0.0 * X;
                                                  output[2] = 0.0 * t;
                                                  return output;
  ...
  // Initialize the visco material's full packed state: {w, previous absolute temperature, Fcm, Fmv, Fcv}.
  auto& state_pred = const_cast<smith::FiniteElementState&>(*system.field_store->getField("state_predicted").get());
  state_pred.setFromFieldFunction([material](smith::tensor<double, dim>) {
    smith::tensor<double, dim, dim> Fcm{};
    smith::tensor<double, dim, dim> Fmv{};
    smith::tensor<double, dim, dim> Fcv{};
    extended_thermomechanics_materials::setIdentity(Fcm);
    extended_thermomechanics_materials::setIdentity(Fmv);
    extended_thermomechanics_materials::setIdentity(Fcv);

    const double initial_entangled_fraction = 0.0;
    const double initial_absolute_temperature = material.Tr;
    return ViscoThermalStiffeningMaterial::FullStatePacking<dim>::pack<Statedim>(
        initial_entangled_fraction, initial_absolute_temperature, Fcm, Fmv, Fcv);
  });
  ...
  std::ofstream file;
  const std::string stress_strain_output = pv_dir + "/_stress_strain_curve_30_c0p1.csv";

  if (mfem::Mpi::Root()) {
    if (axom::utilities::filesystem::makeDirsForPath(pv_dir) != 0) {
      MFEM_ABORT("Could not create ParaView output directory");
    }
    file = std::ofstream(stress_strain_output);
    if (!file.is_open()) {
      MFEM_ABORT("Could not open stress-strain CSV output file");
    }
  ...
  std::cout << "Material model: visco-thermal-stiffening\n";
  ...
  std::cout << "Wrote stress/strain CSV to '" << stress_strain_output << "'\n";
  return 0;
}
...
int main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      std::cout << "Usage: Visco_PNC_driver [--nx=<int>] [--ny=<int>] [--nz=<int>] [--dt=<real>] [--T=<real>] "
                   ...
                   "schur-full]\n";
      std::cout << "Defaults: nx=4 ny=4 nz=1 dt=0.5 T=500.0 solver=strumpack "
  ...
  auto mesh = std::make_shared<smith::Mesh>(
      mfem::Mesh::MakeCartesian3D(num_elements_x, num_elements_y, num_elements_z, mfem_shape, length, width, height),
      "mesh", 0, 0);
  ...
  mesh->addDomainOfBoundaryElements("back", smith::by_attr<example_etm::dim>(1));    // -z
  mesh->addDomainOfBoundaryElements("bottom", smith::by_attr<example_etm::dim>(2));  // -y
  mesh->addDomainOfBoundaryElements("front", smith::by_attr<example_etm::dim>(6));   // +z
  mesh->addDomainOfBoundaryElements("left", smith::by_attr<example_etm::dim>(5));    // -x
  mesh->addDomainOfBoundaryElements("right", smith::by_attr<example_etm::dim>(3));   // +x
  mesh->addDomainOfBoundaryElements("top", smith::by_attr<example_etm::dim>(4));     // +y

  return example_etm::runExtendedThermomechanics(mesh, dt, T, solver_type,
