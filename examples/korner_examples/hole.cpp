...
struct solve_options {
  std::string simulation_tag = "hole";
...
void hole(const solve_options& so)
{
  // Creating DataStore
  ...
  if (so.enable_contact) {
    ...
    solid_solver->addContactInteraction(contact_interaction_id_1, {sideset1}, {sideset1}, so.contact_options);
...
int main(int argc, char* argv[])
{
  ...
  so.nonlinear_options = serac::NonlinearSolverOptions{//.nonlin_solver  = serac::NonlinearSolver::Newton,
                                                       ...
                                                       .print_level = 1};
  so.mesh_location = SERAC_REPO_DIR "/data/meshes/hole.g";
  so.simulation_tag = "hole";
  so.parallel_refinement = 1;
  so.max_time = 1.5;
  so.N_Steps = 300;
  so.strain_rate = -5.0e-1;
