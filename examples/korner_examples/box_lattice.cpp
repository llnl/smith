...
int main(int argc, char* argv[])
{
  ...
  solve_options so;
  BoxMesh tet_hex = BoxMesh::TetMesh2x2;
  so.enable_contact = false;

  switch (tet_hex) {
    case BoxMesh::TetMesh5x5: {
      so.mesh_location = SERAC_REPO_DIR "/data/meshes/box_lattice.g";
      so.simulation_tag = "boxlattice_squish_tet_5x5";
      so.serial_refinement = 1;
      so.parallel_refinement = 1;
      break;
    }
    case BoxMesh::HexMesh5x5: {
      so.mesh_location = SERAC_REPO_DIR "/data/meshes/box_lattice_hex.g";
      so.simulation_tag = "boxlattice_squish_hex_5x5";
      so.serial_refinement = 0;
      so.parallel_refinement = 0;
      break;
    }
    case BoxMesh::TetMesh2x2: {
      so.mesh_location = SERAC_REPO_DIR "/data/meshes/box_lattice_tet_2x2.g";
      so.simulation_tag = "boxlattice_squish_tet_2x2";
      so.serial_refinement = 1;
      so.parallel_refinement = 1;
      break;
    }
    default: {
      MFEM_ABORT("Unimplemented Mesh");
      break;
    }
  }
