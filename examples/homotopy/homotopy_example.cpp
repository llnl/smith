...
int main(int argc, char* argv[])
{
  ...
  mass_objective.addBodyIntegral(serac::DependsOn<1>{}, mesh->entireBodyName(), [](double /*t*/, auto /*X*/, auto RHO) { return get<serac::VALUE>(RHO); });
  ...
  for (int i = 0; i < dim; ++i) {
    auto cg_objective = std::make_shared<ObjectiveT>("translation " + std::to_string(i), mesh, all_states[SHAPE_DISP]->space(), param_space_ptrs);
  ...
  for (int i = 0; i < dim; ++i) {
    auto center_rotation_objective =
        std::make_shared<ObjectiveT>("rotation" + std::to_string(i), mesh, all_states[SHAPE_DISP]->space(), param_space_ptrs);
...
{
  MFEM_VERIFY(x.Size() == dimx && y.Size() == dimy && qeval.Size() == dimy, "InertialReliefProblem::Q -- Inconsistent dimensions");
  qeval = 0.0;
...
mfem::HypreParMatrix * InertialReliefProblem::DxF([[maybe_unused]] const mfem::Vector& x, [[maybe_unused]] const mfem::Vector& y)
{
  return dFdx;
}


mfem::HypreParMatrix * InertialReliefProblem::DyF([[maybe_unused]]const mfem::Vector& x, [[maybe_unused]]const mfem::Vector& y)
{
  return dFdy; 
}


mfem::HypreParMatrix * InertialReliefProblem::DxQ([[maybe_unused]]const mfem::Vector& x,[[maybe_unused]] const mfem::Vector& y)
{
  return dQdx; 
}


mfem::HypreParMatrix * InertialReliefProblem::DyQ([[maybe_unused]]const mfem::Vector& x,[[maybe_unused]] const mfem::Vector& y)
{
