...
namespace example_iterative_etm {
...
smith::LinearSolverOptions linear_options{.linear_solver = smith::LinearSolver::GMRES,
                                          .relative_tol = 1e-8,
                                          .absolute_tol = 1e-8,
                                          .max_iterations = 200,
                                          .print_level = 1};
...
int runIterativeCoupledWithState(const std::shared_ptr<smith::Mesh>& mesh)
{
  double rho = 1.0;
  double E0 = 100.0;
  double nu = 0.25;
  ...
  auto update_rule = [](smith::IterativeExtendedThermoMechanicsTimeIntegrator::Context& ctx) {
    ...
    static const int num_it = 1;
    for (int it = 0; it < num_it; ++it) {
      ctx.solveThermal(1.0);  // demonstrate custom relaxation for one equation
      ctx.solveSolid();
      ctx.solveState();
    }
  };
  ...
  std::string pv_dir = "paraview_iterative_extended_thermomechanics";
...
int main(int argc, char** argv)
{
  ...
  auto mesh = std::make_shared<smith::Mesh>(
      ...
      "mesh", 0, 0);
  mesh->addDomainOfBoundaryElements("left", smith::by_attr<example_iterative_etm::dim>(3));
  mesh->addDomainOfBoundaryElements("right", smith::by_attr<example_iterative_etm::dim>(5));

  return example_iterative_etm::runIterativeCoupledWithState(mesh);
}
